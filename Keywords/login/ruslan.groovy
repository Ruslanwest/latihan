...
class ruslan {
	...
	def login() {
		...
		def data = TestDataFactory.findTestData('Data Files/user-name') // Ganti nama_file_data_anda dengan nama file yang sebenarnya
		def username = data.getValue('user-name', 1)  // Ambil nilai username dari baris pertama
		def password = data.getValue('password', 1)  // Ambil nilai password dari baris pertama
		...
		WebUI.waitForElementVisible(findTestObject('Object Repository/Swag Labs/Page_Swag Labs/span_Products'), 6);
		...
		if (actualText.equals('Products')) {
			...
			WebUI.comment('Login Tidak  Berhasil');
